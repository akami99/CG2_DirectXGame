...
void GamePlayScene::Initialize() {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    ...
    if (i == 7) {
      ...
      newSprite->SetTranslate({100.0f, 100.0f});
    }
...
void GamePlayScene::Draw() {
  // MyGame::Draw の中身を移動
  // ※ dxBase_->PreDraw() などは MyGame
  // 側で行うので、ここでは「シーンの中身の描画」だけを行う
...
void GamePlayScene::UpdateImGui() {
  ...
  // ウィンドウの開始
  ImGui::Begin("Sprite");
  Sprite *sprite = sprites_[9].get();

  Vector2 spritePosition = sprite->GetTranslate();

  if (ImGui::SliderFloat2("translateSprite", &spritePosition.x, 0.0f, 1000.0f,
      "%.1f")) {
  ...
  if (ImGui::TreeNode("Light Settings")) {
    if (ImGui::TreeNode("DirectioalLight_")) {
      ...
      Vector4 &color =
          LightManager::GetInstance()->GetDirectionalLightData().color;
      Vector3 &direction =
          LightManager::GetInstance()->GetDirectionalLightData().direction;
      float &intensity =
