...
bool Sound::LoadWave(const std::string &filePath) {
  ...
  // WAVEFORMATEX
  // は固定サイズではないため、実際のフォーマットデータのサイズを読み込む
  // ここでは、format.chunk.size が WAVEFORMATEX
  // のサイズを超えないことをアサート 厳密には、WAVEFORMATEX
  // の後に拡張データがある場合もあるが、ここでは簡易的にチェック
  ...
  if (strncmp(data.id, "JUNK", 4) == 0) {
    file.seekg(data.size,
               std::ios_base::cur); // JUNK チャンクのサイズ分だけシーク
    file.read(reinterpret_cast<char *>(&data),
              sizeof(data)); // 次のチャンクを読み込む
  }
  ...
  file.read(reinterpret_cast<char *>(pBuffer),
            bufferSize); // 音声データを読み込む
...
// WaveFormatEx のゲッター
const WAVEFORMATEX &Sound::GetWaveFormatEx() const { return wfex; }

// バッファのゲッター
BYTE *Sound::GetBuffer() const { return pBuffer; }

// バッファサイズのゲッター
unsigned int Sound::GetBufferSize() const { return bufferSize; }